...
function setDetailsTable(data) {
    ...
    for (var i = 0; i < data.Details.length; i += 1) {
        ...
        $('#tblVoucherDetailsBody').append(
          '<tr id="' + item.ID + '" data-itemjson="' + JSON.stringify(item) + '" style="cursor:pointer">' +
            '<td>' + item.Text + '</td>' +
            '<td>' + item.Amount + '</td>' +
    ...
    $('#tblVoucherDetailsBody tr').click(function (e) {
        var item = JSON.parse($(e.currentTarget).attr("data-itemjson"));
        console.info("selecting row in voucherDetails with id=" + item);
    });
}

function detailSelected(item) {
